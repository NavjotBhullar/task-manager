...
async def process_notifications(notification_id: str):
    try:
        ...
        if not doc or doc["status"] != NotificationStatus.SENT:
            return
        ...
        if  success:
            await notifications_collection.update_one(
                {"_id": ObjectId(notification_id)},
                {"$set": {"status": NotificationStatus.SENT, "sent_at": datetime.utcnow(), "error_message": None}}
            )
            ...
            await notifications_collection.update_one(
                {"_id": ObjectId(notification_id)},
                {"$set": {"status": new_status, "error_message": error_msg, "retry_count": retry_count}}
            )
